...
import {ActionCreator, FSAAction, isType} from "./action-creators";
...
export interface ReducerBuilder<InS extends OutS, OutS> {
    ...
    caseWithAction<P>(actionCreator: ActionCreator<P>,
                      handler: Handler<InS, OutS, FSAAction<P>>,): ReducerBuilder<InS, OutS>;
    ...
    (state: InS, action: NgRxStoreAction): OutS;
}
...
interface Case<InS extends OutS, OutS, P> {
    ...
    handler: Handler<InS, OutS, FSAAction<P>>;
}
...
function makeReducer<InS extends OutS, OutS>(initialState?: InS): ReducerBuilder<InS, OutS> {
    ...
    reducer.caseWithAction = <P>(actionCreator: ActionCreator<P>,
                                 handler: Handler<InS, OutS, FSAAction<P>>,) => {
    ...
    reducer.case = <P>(actionCreator: ActionCreator<P>, handler: Handler<InS, OutS, P>) =>
        reducer.caseWithAction(actionCreator, (state, action) => handler(state, action ? action.payload : undefined));
...
function getReducerFunction<InS extends OutS, OutS>(initialState: InS | undefined,
                                                    cases: CaseList<InS, OutS>,) {
    return (state = initialState as InS, action: NgRxStoreAction) => {
